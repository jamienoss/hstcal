...
int CalWf3Run (char *input, int printtime, int save_tmp, int verbose, int debug, int onecpu, int fastCTE) {
	...
	int LoadAsn (AsnInfo *);
	int ProcessCCD (AsnInfo *, WF3Info *, int *, int, int, int);
   ...
   char suffix_flc[]="_flc.fits";
	...
	if (asn.detector == CCD_DETECTOR ) { /* Process CCD data ... */
		...
		if (ProcessCCD (&asn, &wf3hdr, &save_tmp, printtime, onecpu, fastCTE)) {
