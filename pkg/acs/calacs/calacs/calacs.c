...
int CalAcsRun (char *input, int printtime, int save_tmp, int verbose, int debug, int onecpu, int gen1cte) {
    ...
    */
    ...
    int LoadAsn (AsnInfo *);
    int ProcessACSCCD (AsnInfo *, ACSInfo *, int *, int, int, int);
    ...
    */
    ...
    if (asn.detector != MAMA_DETECTOR ) {  /* Process CCD data ... */
        ...
        if (ProcessACSCCD(&asn, &acshdr, &save_tmp, printtime, onecpu, gen1cte)) {
...
int ProcessACSCCD (AsnInfo *asn, ACSInfo *acshdr, int *save_tmp, int printtime, int onecpu, int gen1cte) {
